dataset=("facebook" "dblp" "flickr" "nethept" "epinions" "youtube" "pokec" "orkut" "livejournal" "friendster" "DBLP_sym" "Youtube_sym" "twitter" "citeseer" "Flickr_sym" "wikitalk" "wikitalkar" "sample" "Twitter" "congress")
#18: twitter
...
ablation_update=0
ablation_add_root=0
if [[ "$dataset_no" == 8 ]] || [[ "$dataset_no" == 11 ]]; then
    batch=8
fi
if [[ "$dataset_no" ==  18 ]]; then
    batch=4
fi
if [[ "$model" == "IC" ]]; then
