...
OUTPUT="/home/cfeng/mRR_Regen/code/log_mine/mine_Twitter_${eta}_b${batch}_eps${eps}_amp${delta_amp}_${model}_${cur_date}.log"
sudo cset proc -s "$set" -e -- ./run -dataset_No "18" -eta_0 "$1" -batch "$batch" -eps "$eps" -times "$start_time" -run_times "$end_time" -Rand_cost "0" -regen "0.6" -delta_amp "$delta_amp" | tee -a "$OUTPUT"